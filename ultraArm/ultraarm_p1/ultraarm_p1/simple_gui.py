...
class WindowNode(Node):
    ...
    def get_initial_coords(self):
        ...
        if result is not None:
            return [[result.x, result.y, result.z, result.rx], self.speed]
        else:
    ...
    def get_initial_angles(self):
        ...
        result = self.wait_for_future(future)
        if result is not None:
            return [result.joint_1, result.joint_2, result.joint_3, result.joint_4]
        else:
    ...
    def update_gui(self):
        ...
        try:
            ...
            if angles:
                ...
                for i, var in enumerate(self.cont_all[:4]):
                    var.set(self.safe_get_angle(self.res_angles, i))
