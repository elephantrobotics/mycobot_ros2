...
namespace ultraarm_p1_moveit2
{
namespace
{
const rclcpp::Logger LOGGER = rclcpp::get_logger("CouplingPlanningRequestAdapter");
...
bool getJointRad(const moveit_msgs::msg::RobotState& state, const std::string& name, double& value)
{
...
bool checkJ2J3(double j2_rad, double j3_rad, const char* label)
{
  ...
  {
    RCLCPP_WARN(LOGGER, "CouplingPlanningRequestAdapter: invalid %s J2/J3 (%.2f, %.2f deg)",
                label, j2_deg, j3_deg);
    return false;
  }
  return true;
...
public:
  ...
  {
    return "Reject plans when J2/J3 violate ultraArm P1 coupled joint limits";
  }
...
CLASS_LOADER_REGISTER_CLASS(ultraarm_p1_moveit2::CouplingPlanningRequestAdapter,
